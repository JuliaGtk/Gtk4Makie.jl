function Makie.backend_display(::GtkBackend, scene::Scene; start_renderloop=true, visible=true, connect=true)
...
function Makie.backend_display(screen::Screen, scene::Scene; connect=true)
    ...
    # pollevents(screen)
    glarea=Gtk4.G_.get_child(screen.glscreen)
    Gtk4.G_.queue_render(glarea)
    return screen
end
...
function Makie.backend_show(::GtkBackend, io::IO, m::MIME"image/png", scene::Scene)
    img = scene2image(scene)
    FileIO.save(FileIO.Stream{FileIO.format"PNG"}(Makie.raw_io(io)), img)
    return
end

function Makie.backend_show(::GtkBackend, io::IO, m::MIME"image/jpeg", scene::Scene)
    img = scene2image(scene)
