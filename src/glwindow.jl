...
Base.zero(::Type{GtkMakie.SelectionID{T}}) where T = SelectionID{T}(T(0), T(0))
...
struct MonitorProperties
    ...
    dpi::Vec{2, Float64}
    monitor::Gtk4.Gdk4.GdkMonitor
end

function MonitorProperties(monitor::Gtk4.Gdk4.GdkMonitor)
    name = Gtk4.Gdk4.G_.get_model(monitor)
    #isprimary = m == Gtk4.Gdk4.G_.get_monitors(d)[1]
    isprimary = true
    r=Gtk4.Gdk4.G_.get_geometry(monitor)
    position = Vec{2, Int}(r.x,r.y)
    physicalsize = Vec{2, Int}(Gtk4.Gdk4.get_width_mm(monitor), Gtk4.Gdk4.get_height_mm(monitor))
    #videomode = GLFW.GetVideoMode(monitor)
    sfactor = Sys.isapple() ? 2.0 : 1.0
    dpi = Vec(r.width * 25.4, r.height * 25.4) * sfactor ./ Vec{2, Float64}(physicalsize)
    #videomode_supported = GLFW.GetVideoModes(monitor)

    MonitorProperties(name, isprimary, position, physicalsize, dpi, monitor)
end

was_destroyed(nw::Gtk4.GtkWindow) = nw.handle == C_NULL

function GLContext()
    context = Gtk4.Gdk4.get_current()
...
function ShaderAbstractions.native_switch_context!(x::Gtk4.GtkWindow)
    glarea = Gtk4.G_.get_child(x)
    Gtk4.G_.make_current(glarea)
end

function ShaderAbstractions.native_context_alive(x::Gtk4.GtkWindow)
    !was_destroyed(x)
end

function destroy!(nw::Gtk4.GtkWindow)
...
function windowsize(nw::Gtk4.GtkWindow)
    glarea = Gtk4.G_.get_child(nw)
    was_destroyed(nw) && return (0, 0)
    return (Gtk4.width(nw), Gtk4.height(nw))
end

function Base.isopen(window::Gtk4.GtkWindow)
